...
import { getFirstDie, getFirstMod } from '/systems/dcc/module/utilities.js'
  ...
  if (pcObject.characters) {
    ...
    if (pcObject.weapons) {
      for (const weapon of pcObject.weapons) {
        ...
        // If the weapon is a Dagger, or a Handaxe, then add (Ranged) version
        if (weapon.name === 'Dagger' || weapon.name === 'Handaxe') {
          pc.items.push({
            name: weapon.name + ' (Ranged)',
            type: 'weapon',
            img: EntityImages.imageForItem('weapon'),
            system: {
              toHit: weapon.attackMod || '+0',
              damage: weapon.attackDamage || (weapon.name === 'Handaxe' ? '1d6' : '1d4'),
              melee: false
            }
          })
        }
    ...
    if (pcObject.weapon && !pcObject.weapons) {
      ...
      // If the weapon is a Dagger or ends with '(as dagger)', also add Dagger (Ranged)
      if (pcObject.weapon === 'Dagger' || pcObject.weapon === 'Handaxe') {
        pc.items.push({
          name: pcObject.weapon + ' (Ranged)',
          type: 'weapon',
          img: EntityImages.imageForItem('weapon'),
          system: {
            toHit: pcObject.attackMod || '+0',
            damage: pcObject.attackDamage || (pcObject.weapon === 'Handaxe' ? '1d6' : '1d4'),
            melee: false
          }
        })
      }
    ...
    if (pcObject.equipment || pcObject.equipment2 || pcObject.equipment3 || pcObject.tradeGood) {
      ...
      if (pcObject.tradeGood) {
        ...
        if (pcObject.tradeGood.startsWith('Pack A & ')) {
          pcObject.tradeGood = pcObject.tradeGood.substring(9)
          addPackA(pc)
        } else if (pcObject.tradeGood.startsWith('Pack B & ')) {
          pcObject.tradeGood = pcObject.tradeGood.substring(9)
          addPackB(pc)
        } else if (pcObject.tradeGood.startsWith('Pack C & ')) {
          pcObject.tradeGood = pcObject.tradeGood.substring(9)
          addPackC(pc)
        } else if (pcObject.tradeGood.startsWith('Pack D & ')) {
          pcObject.tradeGood = pcObject.tradeGood.substring(9)
          addPackD(pc)
        } else if (pcObject.tradeGood.startsWith('Pack E & ')) {
          pcObject.tradeGood = pcObject.tradeGood.substring(9)
          addPackE(pc)
        }
        pc.items.push({
...
function _parsePlainPCToJSON (pcString) {
  ...
  pcObject.equipment = null // _firstMatch(pcString.match(/Equipment:\s+(.*)[;\n$]/))
...
/**
 * Add Pack A items to character
 * @param {Object} pc - The player character object
 */
function addPackA (pc) {
  // Add Pack A items
  pc.items.push({
    name: 'Armored Jacket',
    type: 'armor',
...
/**
 * Add Pack B items to character
 * @param {Object} pc - The player character object
 */
function addPackB (pc) {
  pc.items.push({
    name: 'Leather',
    type: 'armor',
    img: EntityImages.imageForItem('armor'),
...
/**
 * Add Pack C items to character
 * @param {Object} pc - The player character object
 */
function addPackC (pc) {
  pc.items.push({
    name: 'Leather',
    type: 'armor',
    img: EntityImages.imageForItem('armor'),
...
/**
 * Add Pack D items to character
 * @param {Object} pc - The player character object
 */
function addPackD (pc) {
  pc.items.push({
...
/**
 * Add Pack E items to character
 * @param {Object} pc - The player character object
 */
function addPackE (pc) {
  pc.items.push({
    name: 'Composite Sport, Light',
    type: 'armor',
    img: EntityImages.imageForItem('armor'),
    system: {
      acBonus: '+3',
